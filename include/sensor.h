...
#include <gpiod.h>      // GPIO library for interrupt handling / GPIO库用于中断处理
#include <functional>   // For std::function callbacks / 用于std::function回调
#include <thread>       // For multi-threading / 用于多线程
#include <deque>        // For data buffers / 用于数据缓冲区
#include <mutex>        // For thread safety / 用于线程安全
#include <condition_variable>  // For signaling (not used directly, but for potential extensions) / 用于信号（未直接使用，但用于潜在扩展）
#include <atomic>       // For atomic flags / 用于原子标志
#include <vector>       // For sample vectors in callbacks / 用于回调中的样本向量
#include <cstdint>      // For uint32_t etc. / 用于uint32_t等
#include <chrono>       // For timeouts / 用于超时
